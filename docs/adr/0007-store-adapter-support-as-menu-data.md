...
Missing or `null` adapter data means all adapters are supported, while an explicit empty list supports none and hides the plugin everywhere. Unresolvable entries are ignored without failing the menu but do not establish support. Adapter hiding is derived from final menu data before Mixins run, so a Mixin may override it; it affects only ordinary menu visibility, not Help interception for a command the user has already invoked.
